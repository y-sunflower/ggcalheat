...
## 1) Basic usage on large daily series
...
## 2) Signed data with a diverging scale
...
## 4) Sparse input + explicit date window
...
## 5) Duplicate dates are summed automatically
...
- Continuous and diverging color behavior is controlled with standard
  `ggplot2` scales.
- For production reports, consider fixed date windows (`start_date`, `end_date`)
